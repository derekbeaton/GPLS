...
ca_preproc_X <- ca_preproc(X)
  M_X <- diag(1/ca_preproc_X$m)
  W_X <- diag(1/ca_preproc_X$w)
  ZX_tilde <- (M_X %^% (1/2)) %*% ca_preproc_X$Z %*% (W_X %^% (1/2))  

ca_preproc_Y <- ca_preproc(Y)
  M_Y <- diag(1/ca_preproc_Y$m)
  W_Y <- diag(1/ca_preproc_Y$w)
...
t((M_X %^% (1/2)) %*% ca_preproc_X$Z %*% W_X %*% plsca_reg_results$p[,1]) %*%
  ((M_Y %^% (1/2)) %*% ca_preproc_Y$Z %*% W_Y %*% plsca_reg_results$q[,1])
...
(plsca_reg_results$tx %*% diag(plsca_reg_results$betas) %*% t(plsca_reg_results$v)) / plsca_reg_results$Y_hat
