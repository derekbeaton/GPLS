...
pls_reg <- function(X, Y, center_X = TRUE, center_Y = TRUE, scale_X = TRUE, scale_Y = TRUE, components = 0, tol = .Machine$double.eps){
  ...
  gpls_reg_results$Y_reconstructed_hat <- gpls_reg_results$Y_reconstructed * matrix(Y_scale,nrow(Y),ncol(Y),byrow=T) + matrix(Y_center,nrow(Y),ncol(Y),byrow=T)
  gpls_reg_results$Y_residual_hat <- gpls_reg_results$Y_residual * matrix(Y_scale,nrow(Y),ncol(Y),byrow=T) + matrix(Y_center,nrow(Y),ncol(Y),byrow=T)
  ...
  rownames(gpls_reg_results$Y_residual_hat) <- rownames(gpls_reg_results$Y_reconstructed_hat) <- rownames(Y)
  colnames(gpls_reg_results$Y_residual_hat) <- colnames(gpls_reg_results$Y_reconstructed_hat) <- colnames(Y)
