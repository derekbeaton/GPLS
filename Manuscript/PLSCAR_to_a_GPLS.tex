...
scores and (2) explained variance; both of which we address here. For
visualization of the component scores---which use
...
correspondence analysis literature (CITE). Additionally, instead of
computing the variance per component or latent variable, we can instead
compute the amount of variance explained by each component in
\(\mathbf X\) and \(\mathbf Y\). To do so we require the sum of the
...
be used, so long as the weight matrices are positive semi-definite. To
note, if alternate row weights (i.e., \({\mathbf M}_{\mathbf X}\) or
...
is true in weighted OLS). Additionally, alternate column weights (i.e.,
...
\citep{greenacrefuzzy}. We introduce how to handle various data types as
Section \label{section:appex} progresses to more complex (and ``real
world'') problems.
...
\caption{\label{tab:unnamed-chunk-3}\label{table:r2ex1} The R-squared values over the four latent variables for both groups and genotypes. The full variance of groups is explained over the four latent variables. The groups explained 0.6471\% of the genotypes.}
\centering
\begin{tabular}{lrr}
\toprule
