...
explicit iterative steps in GPLS-COR.
...
\({\mathbf L}_{\mathbf X}\) and \({\mathbf L}_{\mathbf Y}\) with the
...
To note, we can show this with the generalized vectors and weight as
...
GPLS-COR---which is the GPLSSVD---provides the basis for the other two
algorithms: both GPLS-REG and GPLS-CAN make use of GPLS-COR (i.e., the
GPLSSVD) with rank 1 solutions iteratively.
