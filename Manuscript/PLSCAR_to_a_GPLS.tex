...
framework, 2) regress (i.e., residualize) covariates out of mixed and
...
\ref{section:Disc} we discuss PLS-CA-R, and show how PLS-CA-R
generalizes the PLS framework to multiple optimization problems,
...
(i.e., treated as predictors) and also makes use of the SVD iteratively
with deflation of both data tables.
...
are the \emph{generalized}

Correspondence analysis (CA) is a technique akin to PCA---originally
designed for the analysis of contingency tables---and analyzes
deviations to independence. See \citet{greenacre_theory_1984},
\citet{greenacre_correspondence_2010-1}, and
\citet{lebart_multivariate_1984} for detailed explanations of CA, and
then see \citet{escofier-cordier_analyse_1965} and
...
the \emph{observed} probability matrix is computed as
...
\end{equation} The \emph{expected} matrix is computed as
\({\mathbf E}_{\mathbf X} = {\mathbf m}_{\mathbf X}{\mathbf w}_{\mathbf X}^{T}\).
The \emph{deviation} from independence matrix is then \begin{equation}
{\mathbf Z}_{\mathbf X} 
...
or ``correct'') for covariates, akin to how residuals are computed in
OLS. We do so with the original \({\mathbf Z}_{\mathbf Y}\) as
...
variations, some caveats, and some additional features We go into more
detail in later sections on larger variations and broader
generalizations.
...
of the iterative process and the deflation steps that does not occur
with just a single pass of the SVD or GSVD (i.e., PLS-correlation
decomposition). This problem poses two issues: (1) visualization of
component scores and (2) explained variance. For visualization of the
component scores---which use \(\widetilde \delta\)---there is an
...
semi-definite (which requires the use of a generalized inverse).
...
\hypertarget{bring-algo-and-other-points-here-but-in-brief.-and-truncate-above.-also-rename}{%
\section{BRING ALGO AND OTHER POINTS HERE BUT IN BRIEF. AND TRUNCATE
ABOVE. ALSO
RENAME}\label{bring-algo-and-other-points-here-but-in-brief.-and-truncate-above.-also-rename}}
